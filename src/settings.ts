/**
 * Settings management for CodeBuddy Agent ACP
 *
...
export const CODEBUDDY_CONFIG_DIR =
 ...
 */
async function loadSettingsFile(filePath: string | null): Promise<CodeBuddySettings> {
  if (!filePath) {
    return {};
  }
...
export function getManagedSettingsPath(): string {
  switch (process.platform) {
    ...
    case "win32":
      return "C:\\Program Files\\CodeBuddy\\managed-settings.json";
    default:
      return "/etc/codebuddy/managed-settings.json";
  }
}
...
/**
 * Manages CodeBuddy settings from multiple sources with proper precedence.
 */
export class SettingsManager {
  private cwd: string;
  private userSettings: CodeBuddySettings = {};
  private projectSettings: CodeBuddySettings = {};
  private localSettings: CodeBuddySettings = {};
  private enterpriseSettings: CodeBuddySettings = {};
  private mergedSettings: CodeBuddySettings = {};
  ...
  private getUserSettingsPath(): string {
    return path.join(CODEBUDDY_CONFIG_DIR, "settings.json");
  }
  ...
  private getProjectSettingsPath(): string {
    return path.join(this.cwd, ".codebuddy", "settings.json");
  }
  ...
  private getLocalSettingsPath(): string {
    return path.join(this.cwd, ".codebuddy", "settings.local.json");
  }
  ...
  private mergeSettings(): void {
    ...
    const merged: CodeBuddySettings = {};
